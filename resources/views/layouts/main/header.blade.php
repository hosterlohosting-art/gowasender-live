         ...
         <ul class="navbar-nav align-items-center ml-auto">
            ...
            @if(Request::is('user/*'))
               ...
               <li class="nav-item dropdown notifications-icon mr-4">
                  <a class="nav-link notification-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            ...
            <li class="nav-item dropdown">
               <a class="nav-link pr-0 dropdown-toggle" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="display: flex; align-items: center;">
                  <span class="avatar avatar-sm rounded-circle mr-3">
                     <img alt="User" src="{{ Auth::user()->avatar == null ? 'https://ui-avatars.com/api/?name=' . Auth::user()->name : asset(Auth::user()->avatar) }}">
                  </span>
                  <div class="profile-section d-none d-lg-block mr-2" style="text-align: left;">
                     <div class="d-flex align-items-center">
                        <span class="profile-name" style="color: #333; font-weight: 600; font-size: 14px;">{{ Auth::user()->name }}</span>
                        <i class="fas fa-chevron-down ml-2" style="font-size: 10px; color: #8898aa;"></i>
                     </div>
                     <span class="profile-role-badge" style="background-color: #e3fcf0; color: #0d885a; font-size: 11px; padding: 2px 8px; border-radius: 4px; font-weight: 600; display: inline-block; line-height: 1.5;">{{ Auth::user()->role == 'admin' ? __('Admin') : __('User') }}</span>
                  </div>
               </a>
