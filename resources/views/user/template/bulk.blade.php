               ...
               <div class="col-sm-4">
                  <div class="card card-stats">
                     <div class="card-body">
                        ...
                        <h5 cl   ass="card-title  text-muted mb-0">{{ __('Total Contacts') }}</h5>
                        <p></p>
                     </div>
                  </div>
               </div>
                              ...
                              @foreach($contacts as $key => $contact)
                                       ...
                                       <input type="hidden" name="contact" value="{{ $contact->id }}">
                                                             @php
                                                               ...
                                                               // Define additional placeholders to replace
                                                               $additionalPlaceholders = ['name', 'phone'];

                                                               // Iterate through param placeholders (param1, param2, ..., param7) and additional placeholders (name, phone)
                                                               for ($i = 1; $i <= 7; $i++) {
                                                                  $paramPlaceholder = "{param$i}";

                                                                  // Replace param placeholders in body and header
                                                                  ...
                                                                  // Replace additional placeholders (name, phone) in body and header
                                                                  foreach ($additionalPlaceholders as $placeholder) {
                                                                     $additionalPlaceholder = "{" . $placeholder . "}";
                                                                     if (strpos($replacedBody, $additionalPlaceholder) !== false) {
                                                                        $replacedBody = str_replace($additionalPlaceholder, $contact->$placeholder, $replacedBody);
                                                                     }

                                                                     if (strpos($replacedHeaderParm, $additionalPlaceholder) !== false) {
                                                                        $replacedHeaderParm = str_replace($additionalPlaceholder, $contact->$placeholder, $replacedHeaderParm);
                                                                     }
                                                                  }
                                                               }
                                                            @endphp
                                      ...
                                      <input type="hidden" name="body" value="{{ $replacedBody }}">
                                       ...
                                       <td>
                                          ...
                                          @if($template->id == 0)
                                             <option value="0" selected>{{ __('Plain Text') }}</option>
                                          @endif

                                                                                @foreach($templates as $template_row)
                                                                                 <option value="{{ $template_row->id }}" {{ ($template->id > 0 && $template_row->id == $template->id) ? 'selected' : ''}}>{{ $template_row->title }}</option>
                                                                              @endforeach
                                          </select>
