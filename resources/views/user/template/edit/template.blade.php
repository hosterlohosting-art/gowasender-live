<form method="POST" action="{{ route('user.template.update',$template->id) }}" class="ajaxform">
   ...
   <div class="col-sm-12">
         <div class="form-group">
            ...
            <input type="text" name="template_name" class="form-control" value="{{ $template->title }}" readonly>
         </div>
      </div>

    <div class="col-sm-12">
    <div class="form-group">
    <label>{{ __('Select Header Type:') }}</label>
    <select name="header_type" id="header_type" class="form-control">
        <option value="none" @if($template->body['headerType'] === 'none') selected @endif>{{ __('No Header') }}</option>
        <option value="text" @if($template->body['headerType'] === 'text') selected @endif>{{ __('Text Header') }}</option>
        <option value="media" @if($template->body['headerType'] === 'media') selected @endif>{{ __('Media Header') }}</option>
    </select>
</div>
</div>

<div class="col-sm-12" id="text_header_div">
    <div class="form-group">
        <label>{{ __('Text Header:') }}</label>
        <input type="text" name="text_header" id="text_header" class="form-control" value="{{ $template->body['headerText'] ?? '' }}">
    </div>
    <!-- Additional Input for Text Header Variables if Needed -->
    @if(!empty($template->body['headerParameters']))
    <div id="text_header_params" class="form-group">
        @foreach($template->body['headerParameters'] as $value)
            <input type="text" name="text_parameter_{{ $loop->index + 1 }}" id="parameter_input_{{ $loop->index + 1 }}" value="{{ $value }}" class="form-control">
        @endforeach
    </div>
@endif
      <div class="col-sm-12">
         ...
         <div class="form-group">
            <textarea class="form-control h-200" name="message" maxlength="1000">{{ $template->body['messageText'] ?? '' }}</textarea>
         </div>
         @if(!empty($template->body['messageParameters']))
         <div id="parameter_fields" class="form-group">
         @foreach($template->body['messageParameters'] as $value)
            <input type="text" name="message_parameter_{{ $loop->index + 1 }}" id="parameter_input_{{ $loop->index + 1 }}" value="{{ $value }}" class="form-control">
        @endforeach
    </div>
    @endif
      </div>
      ...
      <div class="col-sm-12">
         <div class="form-group">
            ...
            <input type="text" class="form-control" name="footer_text" required="" autofocus="" maxlength="100" value="{{ $template->body['footerText'] ?? '' }}" />
         </div>
      </div>
