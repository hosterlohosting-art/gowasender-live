...
$('.csv').on('change',function(){
	...
	reader.onload = function(event){
		...
		let cloudapi=$('#device_import').val();
		let cloudapi_number=$("#device_import option:selected").text();
		var isCrossed = false;
		...
		$.each(data, function (key, item) 
		{
			...
			if (is_valid == true) {
				...
				<td class="col-2">
				${cloudapi_number}
				<input type="hidden" name="cloudapi" value="${cloudapi}" class="device_${key}" required="">
				</td>
...
$(document).on('click','.add_custom_row',function(){
	...
	let cloudapi=$('#device_custom').val();
	let cloudapi_number=$("#device_custom option:selected").text();;
		...
		$(numbersArr).each(function (numberKey, number){
			...
			if (!isNaN(parsedPhone) && parsedPhone.toString() === number) {
				...
				if (isExist(parsedPhone)) {
					...
					<td class="col-2">
					${cloudapi_number}
					<input type="hidden" name="cloudapi" value="${cloudapi}" class="device_${rows}" required="">
					</td>
...
$('.contact-list-form').on('submit',function(e){
	...
	let cloudapi=$('#wa-device').val();
	let cloudapi_number=$("#wa-device option:selected").text();
	var isCrossed = false;
	...
	$.each(contactsNumbers, function (key, item) 
	{
		...
		if (is_valid == true) {
			...
			<td class="col-2">
			${cloudapi_number}
			<input type="hidden" name="cloudapi" value="${cloudapi}" class="device_${key+1000}" required="">
			</td>
...
function submitRequests(param) {
	...
	const cloudapi=$('.device_'+param).val();
	...
	if (cloudapi == '') {
		$('.send_now').removeAttr('disabled');
		return ToastAlert('error', 'Select a cloudapi');
	}
	...
	$.ajax({
		type: 'POST',
		url: form_link,
		data: {phone: phone_num, cloudapi:cloudapi,message:message},
		dataType: 'json',
		beforeSend: function() {
