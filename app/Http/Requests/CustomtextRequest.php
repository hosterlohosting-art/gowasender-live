...
class CustomtextRequest extends FormRequest
{
    ...
    public function rules(Request $request,$type)
    {
        ...
        if ($type == 'text-with-media') {
            ...
            $file=$this->saveFile($request,'image');
            $request['attachment']=$file;
                        ...
                        if (!in_array($buttonKey, $properties)) {
                            ...
                            if($buttonKey == 'action'){
                                ...
                                if (!empty($buttonValue)) {
                                    if (strlen($buttonValue) > 20) {
                                        $error_message=__('Maximum Button Value Limit 20');
                                        $is_valid = false;
                                    }
                                }
        ...
        elseif ($type == 'text-with-list') {
            ...
            foreach ($request->section as $key => $section) {
               ...
               elseif (strlen($section['title']) > 20) {
                   $is_valid=false;
                   $error_message=__('Maximum title limit is 20');
                ...
                foreach ($section['value'] as $value_key => $value) {
                    ...
                    elseif (strlen($value['title']) > 20) {
                     $is_valid=false;
                     $error_message=__('List value name maximum word limit is 20');
