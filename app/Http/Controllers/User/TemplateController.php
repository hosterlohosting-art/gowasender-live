...
class TemplateController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('user.template.index', compact('templates', 'active_templates', 'inactive_templates', 'total_templates', 'limit', 'cloudapis'));
    }
    ...
    public function loadTemplate(Request $request)
    {
        ...
        $businessId = $cloudapi->wa_business_id;


        try {
            DB::beginTransaction();

            $response = $whatsapp_app_cloud_api->loadTemplatesFromWhatsApp($businessId, $accessToken);

            foreach ($response['data'] as $templateData) {
                $uuid = $templateData['id'];

                // Check if a template with the same UUID already exists
                $existingTemplate = Template::where('uuid', $uuid)->first();
                ...
                if ($templateData['status'] == 'APPROVED') {
                    ...
                    $template->cloudapi_id = $request->cloudapi_id;
                    $template->title = $templateData['name'];
                    $template->body = json_encode($templateData); // Convert to JSON format
            ...
            DB::commit();
            return response()->json(['message' => 'Templates loaded successfully.'], 200);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function getTemplate(Request $request)
    {
        ...
        foreach ($response['data'] as $templateData) {
            ...
            if ($templateData['status'] !== 'APPROVED') {
                // Skip if the template is not approved
                continue;
            }
            ...
            $this->saveCloudTemplate($templateData, 'meta-template', Auth::id(), $templateStatus, $request->cloudapi_id);
    ...
    {
        $cloudapis = CloudApi::where('user_id', Auth::id())->where('status', 1)->latest()->get();
        return view('user.template.create', compact('cloudapis'));
    }
    ...
    public function store(Request $request, $type)
    {
        ...
        $validated = $request->validate(['template_name' => 'required|max:100',]);
        if ($type == 'plain-text') {
            $validated = $request->validate(['message' => 'required|max:1000',]);
            ...
            $cloud_id = $cloudapi->id;
            $whatsapp_app_cloud_api = new WhatsappLibrary();
            $accountId = $cloudapi->wa_business_id;
            ...
            if ($request->buttons[0]['type'] != 'none') {
            ...
            $templateRawData = $request->all();
            if ($request->hasFile('file')) {
                $file = $request->file('file');
                $filePath = $file->getPathname();
                $media = $whatsapp_app_cloud_api->uploadProfilePicture($filePath, $appId, $accessToken);
                $header = $whatsapp_app_cloud_api->getHeaderTextComponent($templateRawData, $media);
            } else {
                $header = $whatsapp_app_cloud_api->getHeaderTextComponent($templateRawData);
            }
            $body = $whatsapp_app_cloud_api->getBodyTextComponent($templateRawData);
            $footer = $whatsapp_app_cloud_api->getFooterTextComponent($templateRawData);
            $buttons = [];
            if ($request->buttons[0]['type'] != 'none') {
                $buttons = $whatsapp_app_cloud_api->getButtonsComponent($templateRawData);
            }
            $templateApiResult = $whatsapp_app_cloud_api->createTemplate($templateRawData, $header, $body, $footer, $buttons, $accountId, $accessToken);

            if ($templateApiResult['status'] != 200 && $templateApiResult['status'] != 201) {
                return response()->json([
                    'message' => $templateApiResult['error'] ?? __('Failed to create template on Meta API'),
                ], 403);
            }

            $templateStatus = $templateApiResult['status'];
            $request['id'] = $templateApiResult['body']['id'] ?? null;
        ...
        if ($type == 'meta-template' && ($templateStatus == 200 || $templateStatus == 201)) {
            $this->saveCloudTemplate($request->all(), $type, Auth::id(), 0, $request->cloudapi);
            return response()->json([
                'message' => __('Template created successfully!!.Waiting for the Approval of Meta'),
            ], 200);
        } else {
    ...
    public function update(Request $request, $id)
    {
        ...
        $type = $template->type;
        $cloudapi = CloudApi::where('id', $template->cloudapi_id)->latest()->first();
        $whatsapp_app_cloud_api = new WhatsappLibrary();
        $accessToken = $cloudapi->access_token;
        ...
        if ($type == 'text-with-media') {
            ...
            $templateUpdateData = $request->all();
            $header = $whatsapp_app_cloud_api->getHeaderTextComponent($templateUpdateData);
            $body = $whatsapp_app_cloud_api->getBodyTextComponent($templateUpdateData);
            $footer = $whatsapp_app_cloud_api->getFooterTextComponent($templateUpdateData);
            $buttons = $whatsapp_app_cloud_api->getButtonsComponent($templateUpdateData);
            $templateUpdateStatus = $whatsapp_app_cloud_api->editTemplate($header, $body, $footer, $buttons, $template_id, $accessToken);
        ...
        if ($type == 'text-with-template') {
            $template = $this->saveCloudTemplate($request->all(), $type, Auth::id(), 0, $id);
        } else {
